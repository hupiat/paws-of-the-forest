...
public abstract class HUD {
    ...
    public static void open(Player player) {
        ...
        remove(player);
        updateInterface(player, 0.5, 0.5, 0.5, 0.5);
    }

    public static void updateInterface(Player player, double thirst, double energy, double hygiene, double social) {
        remove(player);

        String barText = createPartialBar(thirst, THIRST_FULL_ICON, THIRST_EMPTY_ICON) + "  " +
                createPartialBar(energy, ENERGY_FULL_ICON, ENERGY_EMPTY_ICON) + "  " +
                createPartialBar(hygiene, HYGIENE_FULL_ICON, HYGIENE_EMPTY_ICON);
        ...
        // Action bar (social)
        String socialText = SOCIAL_ICON + "  " + (int)(social * 100) + "%";
        new BukkitRunnable() {
            @Override
            public void run() {
                if (!player.isOnline()) {
